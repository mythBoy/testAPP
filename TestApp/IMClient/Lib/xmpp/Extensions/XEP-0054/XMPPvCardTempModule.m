...
// fetch vcard if timeout . add by pengjay
//		if (vCardTemp == nil && shouldFetch && [_xmppvCardTempModuleStorage shouldFetchvCardTempForJID:jid xmppStream:xmppStream])
		if (vCardTemp == nil||vCardTemp.nickname.length <= 0||(shouldFetch && [_xmppvCardTempModuleStorage shouldFetchvCardTempForJID:jid xmppStream:xmppStream]))
		{
			[self _fetchvCardTempForJID:jid];
		}
...
{
	[self vCardTempUpdateFromServerForJID:[xmppStream myJID]];
}
