...
#import "NewWeiBo.h"
static NSString *abcdefg =@"红咔嚓雷bo";
...
//- (void)setWeibo:(NewWeiBo *)weibo
-(void)setDict:(NSDictionary *)dict
{
    _dict = dict;
    
    // 1 获取数据
    NSString *text = dict[@"address"];
//    NSString *icon = [testUtil isHttpNewProJ:dict[@"thum"]];
    
    NSString *name = dict[@"merchant_name"];
    
    NSString *picture = dict[@"thumb"] ?[testUtil isHttpNewProJ:dict[@"thumb"]] : nil;
    ...
    // 计算文字的宽高
    CGSize nameSize = [self sizeWithString:name font:NJNameFont maxSize:CGSizeMake(MAXFLOAT, MAXFLOAT)];
    ...
    CGSize textSize =  [self sizeWithString:text font:NJTextFont maxSize:CGSizeMake(300, MAXFLOAT)];
