...
@property (nonatomic, strong) UILabel *loadMoreData;
...
//    NSLog(@"===infinite====》%f",contentOffset.y);
...
//            self.state = SVInfiniteScrollingStateLoading;
            if (self.scrollView.contentOffset.y>0) {   //修改  by：李洋 2015/12/31
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        CGPoint origin = CGPointMake(roundf((self.bounds.size.width-viewBounds.size.width)/2), roundf((self.bounds.size.height-viewBounds.size.height)/2));
        [self.activityIndicatorView setFrame:CGRectMake(origin.x-55, origin.y, viewBounds.size.width, viewBounds.size.height)];

        if (!_loadMoreData) {
            self.loadMoreData = [[UILabel alloc] init];
        }
        _loadMoreData.frame = self.activityIndicatorView.frame;
        _loadMoreData.backgroundColor = [UIColor clearColor];
        _loadMoreData.left = origin.x-20;
        _loadMoreData.textColor = [UIColor grayColor];
        _loadMoreData.width = 120;
        [self addSubview:_loadMoreData];
//        _loadMoreData.text = nil;
        _loadMoreData.font = [UIFont systemFontOfSize:14];
